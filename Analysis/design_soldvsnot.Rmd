...
parms4 <- resdf  %>% filter(maxydiff < 30) %>% filter(d2p>.5 & d2p_i>.5) %>% filter(effn==max(effn)) %>% filter(x2==max(x2))
apply(parms4,2,summary)
parms4

## making parms3 == parms4 for now for ease below
parms3 <- parms4
