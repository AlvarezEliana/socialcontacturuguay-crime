...
REORGANIZE NOW THAT WE ARE DOING THE SEARCH
...
```{r}
load(here::here("Analysis", "design_soldvsnot_search_res2.rda"))
```
...
with(resdf, plot(crime_p.value, effn, pch = 21, bg = gray(maxydiff / max(maxydiff),alpha=.7),cex=.7))
with(resdf[resdf$effn>19,], text(crime_p.value,effn,labels = n,offset=.2,pos=2,cex=.7))
```

```{r}
pdf(file="designps.pdf")
gp3 <-  ggplot(data=resdf,aes(x=crime_p.value,y=d2p_i,color=effn)) +
	geom_point()
print(gp3)
dev.off()
```
...
parms1 <- resdf %>% dplyr::filter(crime_p.value >= .6)  %>% filter(effn==max(effn)) %>%  filter(d2p_i==max(d2p_i)) %>%  filter(x2==max(x2) & x3==max(x3)) %>% unique
parms1

## Now max d2p_i
parms2 <- resdf  %>% filter(d2p_i>quantile(d2p_i,.95)) %>% filter(crime_p.value>quantile(crime_p.value,.9)) %>% filter(effn==max(effn)) %>%
	filter(x2==max(x2) & x3==max(x3)) %>% unique
parms2

## Now focus on max differences
parms3 <- resdf  %>% filter(maxydiff < 30) %>% filter(crime_p.value > .1) %>% filter(effn==max(effn)) %>% filter(x2==max(x2))
parms3
