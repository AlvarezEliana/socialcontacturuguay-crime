...
## pdist <- match_on(soldvsnot17~pscore1+pscore2,data=dat17p,method="rank_mahalanobis")
pdist <- match_on(soldvsnot17 ~ pscore1+pscore2, data = dat17p) ## want  pscore2 to play a bigger role, method = "rank_mahalanobis")
...
crimecal <- quantile(as.vector(crimedist), .95)
...
```{r penalizedistmats}
mhdistPen <- mhdist + (pdist > pcal) * maxmh + (mhdist > mcal) * maxmh + (crimedist > crimecal)*100*maxmh ## Penalty like Rosenbaum
psdistPen <- pdist + (pdist > pcal) * maxps + (mhdist > mcal) * maxps + (crimedist >  crimecal)*100*maxps ## Penalty like Rosenbaum
...
find_design(
  x = c(0, max(mhdistPen), max(psdistPen)),
...
search_space <- as.matrix(expand.grid(
  ...
  mhcal = quantile(mhdistPen, seq(.5, 1, length.out = 40)),
...
tryCatch(load(here::here("Analysis", "design_soldvsnot_search_res.rda")),
  error = function(e) {
...
cut1 <- resdf %>% dplyr::filter(d2p >= .7 & d2p_i >= .6)
...
parms2 <- resdf  %>% filter(d2p>quantile(d2p,.95)) %>% filter(d2p_i>quantile(d2p_i,.9)) %>% filter(effn==max(effn)) %>%
	filter(x2==max(x2)) %>% unique
parms2

## Now almost max d2p_i (turns out no dropping obs in this design)
parms3 <- resdf  %>% filter(d2p_i>.7) %>% filter(d2p==max(d2p)) %>% filter(x3==max(x3) & x1==max(x1) & x2==max(x2))
...
newdist1 <- (psdistPen * parms1[[1]] + mhdistPen * (1 - parms1[[1]])) + caliper(mhdistPen, parms1[[2]]) + caliper(psdistPen, parms1[[3]])
...
newdist2 <- (psdistPen * parms2[[1]] + mhdistPen * (1 - parms2[[1]])) + caliper(mhdistPen, parms2[[2]]) + caliper(psdistPen, parms2[[3]])
...
newdist3 <- (psdistPen * parms3[[1]] + mhdistPen * (1 - parms3[[1]])) + caliper(mhdistPen, parms3[[2]]) + caliper(psdistPen, parms3[[3]])
...
ydiffs1 <- unlist(matched.distances(fm1, distance = crimedist))
summary(ydiffs1)
...
## Match 2:
fm2 <- fullmatch(newdist2, data = dat17p, tol = .00001, min.controls = 1)
...
ydiffs1 <- unlist(matched.distances(fm2, distance = crimedist))
summary(ydiffs1)
## Do individual level balance test
ndati1 <-  nrow(dat17i)
dat17i$fm2 <- NULL
...
ydiffs3 <- unlist(matched.distances(fm3, distance = crimedist))
summary(ydiffs3)
## Do individual level balance test
ndati1 <-  nrow(dat17i)
dat17i$fm3 <- NULL
...
save(fm1, fm2, fm3,
     ...
     parms1, parms2, parms3, xbi, xb123, dat17i, dat17p, file = here::here("Analysis","design_soldvsnot.rda"))
