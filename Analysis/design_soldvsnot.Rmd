...
parms1 <- resdf %>% filter(maxTp > quantile(maxTp,.5) & crime_p.value > quantile(crime_p.value,.5) & effn >= 16)  %>%
    filter(maxbdiff < median(maxbdiff) & maxadiff < median(maxadiff) &
        abs(robbdiff) < 2 & abs(vrobbdiff) < 2)  %>% filter(effn==max(effn))
parms1
...
parms3 <- parms3 %>% filter(x2==max(x2))
