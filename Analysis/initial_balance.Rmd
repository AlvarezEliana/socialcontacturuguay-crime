...
tmp  <-  model.matrix(update(baselineFmlaCluster,.~.-1),data=dat17i)
uniqX <-  sort(apply(tmp,2,function(x){ length(unique(x)) }))
uniqX[1:10]
anyNA <-  apply(tmp,2,function(x){  any(is.na(x)|is.nan(x)) })
table(anyNA)
