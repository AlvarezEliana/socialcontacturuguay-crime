...
```{r}
## Leaving this code for now. I think that we have too many cluster-level
## variables here and are getting p-values from a degenerate distribution.
##  The calculations of the standardized differences are fine. So we will use
##  this code for possible descriptive plots.
...
A couple of other approaches to summarizing the covariate-to-treatment
relationships using a single p-value.

First, at the individual level with cluster-adjusted statistical inferences

```{r}

num_singular_values <- 20
   mm <-  model.matrix(update(baselineFmla,.~.-1),data=dat17i)
   thesvd <- svd(mm,nu=num_singular_values)$u
   colnames(thesvd) <- paste("V",1:ncol(thesvd),sep="")
   newdat <- data.frame(dat17i,thesvd)
   xb_i_fmla <- reformulate(colnames(thesvd),response="soldvsnot17")
    xb_svd_i <- balanceTest(update(xb_i_fmla, . ~ .  + cluster(Q56)),
      data = newdat)
xb_svd_i$overall[,]

```

Next  at level of pharmacy:
...
## Degenerate p-value
xb0p <- balanceTest(matchfmla,data=dat17p,report="all")

## Another way to see the same result
initial_neigh_bal_fmla <- ~soldvsnot17
lhs(initial_neigh_bal_fmla) <- rhs(matchfmla)
initial_omnibus_bal <-
...
```{r}
save(xb0i, xb_svd_i, initial_omnibus_bal, initial_omnibus_bal_perm, baselineFmla, baselineFmlaCluster, file = "initial_balance.rda")
```
