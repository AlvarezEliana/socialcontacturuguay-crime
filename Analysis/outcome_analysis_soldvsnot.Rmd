...
regcovs <- c("ideol_si_i","educ_i","sex_i","age_i")
...
```{r}
outdat2 <- finaldat %>% dplyr::select(c(regdesignvars,regoutcomes,regcovs)) %>% filter(ronda==2018)

outdat2 <- outdat2 %>% mutate_if(is.character, as.numeric)
...
outdat3 <- outdat2 %>% dplyr::select(-c("id","age_i")) %>% mutate_all(replace_NA_0)
stopifnot(sum(is.na(outdat3$boca1_i))==288) ## make sure to preserve missings
outdat3$id <- outdat2$id
outdat3$age_i <- outdat2$age_i
...
```{r}
designdat <- dat17p %>% dplyr::select(c("Q56","fm2","soldvsnot17","ph_type","pscore1","pscore2"))
...
outdat5 <- outdat4 %>% filter(!is.na(fm2)) %>% ungroup()
dim(outdat5)
...
## 	x - mean(x)
## }
## covdat <- optmatch::fill.NAs(outdat5[,c("fm2",regcovs)])
## covdat <- covdat %>% group_by(fm2) %>% mutate_all(block_center)
...
## Test the function.
est1fn("dt_impact_i",dat=outdat5,blocks="fm2",clusters="Q56",weights="nbwt")
est1fn("dt_impact_i",covnms=regcovs,blocks="fm2",dat=outdat5,clusters="Q56",weights="nbwt")
## Next two are precision weights.
est1fn("dt_impact_i",dat=outdat5,blocks="fm2",clusters="Q56",weights="hbwt")
est1fn("dt_impact_i",dat=outdat5,blocks="fm2",clusters="Q56",fixed_effects=~fm2)
est1fn("dt_impact_i",covnms=regcovs,dat=outdat5,blocks="fm2",clusters="Q56",weights="hbwt")
est1fn("dt_impact_i",covnms=regcovs,dat=outdat5,blocks="fm2",clusters="Q56",fixed_effects=~fm2)

## Compare to the following test
test1 <- balanceTest(soldvsnot17~dt_impact_i+strata(fm2)+cluster(Q56),data=outdat5,report="all")
test1$results[,,"fm2"]
test1rank <- balanceTest(soldvsnot17~dt_impact_i+strata(fm2)+cluster(Q56),data=outdat5,report="all",post.alignment.transform=rank)
test1rank$results[,,"fm2"]

## Getting missingness warnings
est1fn("boca1_i",dat=outdat5,blocks="fm2",clusters="Q56",weights="nbwt")
...
res1 <- lapply(regoutcomes,function(ynm){
		       message(ynm)
		       unadj <- est1fn(ynm,dat=outdat5,blocks="fm2",clusters="Q56",weights="nbwt")
		       covadj <- est1fn(ynm,covnms=regcovs,blocks="fm2",dat=outdat5,clusters="Q56",weights="nbwt")
		       tmp <- bind_cols(unadj,covadj)
				 })
