...
<!-- NOTE: Adjust for baseline crime covariates within set because of large variability across pharmacies in baseline crime. This may not be necessary now that we've done individual matching -->
...
# Not clear we will do this. Let's do the simple
## unadjusted approach first.
...
designdat_p <- dat17p %>% dplyr::select(c(
  "Q56", "parms1_fm_p", "soldvsnot17", "ph_type", "pscore1", "pscore2",
...
## Add the individual level matching information
## I forgot that we had different people available in 2018.
designdat_i <- dat17i %>% dplyr::select(c("id",
  "Q56", "parms1_fm_p","parms1_fm_i", "soldvsnot17", "ph_type",
  "n_sec_i", "robb_2016", "vrobb_2016"
))

outdat6 <- left_join(outdat5,designdat_i,by="id",keep=TRUE) %>% droplevels()
...
lm_robust(n_sec_i ~ soldvsnot17, dat = tmpdat, fixed_effects = ~parms1_fm_p, clusters = Q56, se_type = "CR0")
lr0a <- lm_robust(n_sec_i ~ soldvsnot17 + parms1_fm_p, dat = tmpdat, clusters = Q56, se_type = "CR0")
...
lm_robust(n_sec_i ~ soldvsnot17 + nclus, fixed_effects = ~parms1_fm_p, dat = tmpdat, clusters = Q56, se_type = "CR0")
lr0b <- lm_robust(n_sec_i ~ soldvsnot17 + nclus + parms1_fm_p, dat = tmpdat, clusters = Q56, se_typ = "CR0")
...
# library(lme4)
# lme1 <- lmer(n_sec_i ~ (1 | Q56) + soldvsnot17+ideol_si_i+educ_i+sex_i+age_i+n_sec_i+robb_2016_mean+vrobb_2016_mean+n_sec_i_mean + nclus  + parms1_fm_p, data=tmpdat)
...
res1 <- lapply(regoutcomes, function(ynm) {
  message(ynm)
  unadj <- est1fn(ynm, trtnm = "soldvsnot17", dat = outdat5, blocknm = "parms1_fm_p", clusternm = "Q56", weights = "nbwt", se_type = "CR0")
  covadj <- est1fn(ynm, trtnm = "soldvsnot17", icovnms = icovs, pcovnms = pcovs, blocknm = "parms1_fm_p", dat = outdat5, clusternm = "Q56", weights = "nbwt", se_type = "CR0")
