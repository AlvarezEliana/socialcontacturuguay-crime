...
---
title: Describe Chosen Research Design --- Sold Vs Not Comparison
author: Jake Bowers
...
## Using the pharmacy level full matching
...
dev.off()
## n_sec neighborhood security perceptions
```
...
## vrobb son violentos (assault)
## robb son robos
```

## Using the MultiMatch input to Fullmatch

```{r}
## Not elegant but don't want to get confused
rm(list=ls())

```

```{r}
load(here::here("Analysis","design_svn_zubi.rda"),verbose=TRUE)
```
...
pdf(file=here::here("media","fm2_mm1_balance_plot.pdf"))
par(oma=rep(0,4)+.01,mar=c(3,8,0,1),mgp=c(1.5,.5,0))
with(xb_i2vars,{
	     plot(abs(std.diff),thevar,
		       pch=21,
...
This makes me think we should use the original matched design.
