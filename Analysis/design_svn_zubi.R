...
tmp2 <- matchfmlacovs[tmp]
names(dat17p)[names(dat17p) %in% tmp2] <- gsub("_mean","",names(dat17p)[names(dat17p) %in% tmp2])
matchfmlacovs[tmp] <- gsub("_mean","",matchfmlacovs[tmp])
matchfmla2 <- reformulate(matchfmlacovs,response="soldvsnot17")

## Assess mm1
mm1p <- mm1_data %>% group_by(Q56) %>% summarize(mm1=unique(pair.id))
...
xb_mm1vars %>% arrange(desc(abs(std.diff))) %>% head(x,n=10)
...
xb_imm1vars %>% arrange(desc(abs(std.diff))) %>% head(x,n=10)
mm1_bal1_schools <- as.data.frame(mm1_bal1$schools) 
mm1_bal1_schools$var <- row.names(mm1_bal1_schools)
mm1_bal1_schools %>% arrange(desc(abs(`SDiff After`))) %>% dplyr::select(matches("SDiff|var")) %>% head(n=10)

xb_mm1vars %>% dplyr::filter(grepl("pt_per|age_av|educ_av",vars))

## Simple full matching as opposed to the pair matching done above
